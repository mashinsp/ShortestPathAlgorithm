...
const square_mouseenter = (event) => {
  let elem = event.target;
  elem.classList.add("highlight");
  draw_path(elem);
}

const square_mouseleave = (event) => {
  event.target.classList.remove("highlight");
}

const square_click = (event) => {
...
const shortest_path = (start, end, max = Infinity) => {
  if (start === end) return [];
  let path = [];
  let c = 0;
...
const get_next_shortest_node = (start, end) => {
...
const change_page = () => {
    const val = document.getElementById('inp-val').innerText;
    localStorage.setItem("gridvalue", val);
    window.location.href = "./index.html";
} 
const change_page_back = () => {
